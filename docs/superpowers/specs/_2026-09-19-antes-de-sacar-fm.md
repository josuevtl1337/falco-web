...
- Pedidos registrados con código y estado ("confirmado", "retirado") para medir la conversión real.
...
-- Claves: hopper_coffee_id, whatsapp_number, menu_url, playlist_url, instagram_url.
```
...
5. **Dónde estamos + Falco FM:** calendario, horarios con la fila de hoy marcada, mapa, "Cómo llegar", "Ver la carta ↗" (lleva a `/carta`), "Escribir por DM", y el reproductor al final.
...
- **Link:** `https://wa.me/{whatsapp_number}?text={mensaje codificado}`. Si no hay nombre, el saludo queda en "¡Hola Falco! Quiero hacer este pedido".
- **Límites:** 20 productos distintos y 2 unidades por producto, para que el link no quede demasiado largo.
...
| `order_resume_chat`                                                                   | —                                           |
| `directions_click` / `menu_click` / `instagram_click` / `playlist_click` / `dm_click` | —                                           |
...
- **Unitarias:** cálculo de "Abierto ahora" (hora de Argentina, días especiales, cierre a medianoche, próxima apertura), armado del mensaje y del link `wa.me`, lógica del pedido (sumar, quitar, límites, productos que desaparecen) y validaciones zod.
...
| Link de la playlist                                     | Falco    | Falco FM        |
